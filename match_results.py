...
file_name = 'ibk-poc-meeting_20250106_1.json'

with open(os.path.join('./meeting_records/diar', 'diar_' + file_name), mode='r', encoding='utf-8') as f:
    diar_data = json.load(f)

with open(os.path.join('./meeting_records/stt', 'stt_' + file_name), mode='r', encoding='utf-8') as f:
...
with open(os.path.join('./meeting_records', 'merged_' + file_name), 'w') as output_file:
    json.dump(diar_data, output_file, ensure_ascii=False, indent=4)

updated_stt_data = []
for stt_segment in stt_data:
    print(stt_segment)
    updated_stt_data.append(map_speaker_with_nested_check(diar_data, stt_segment))

# print(updated_stt_data)

# 결과 저장
with open(os.path.join('./meeting_records', 'stt-diar_' + file_name), 'w') as output_file:
